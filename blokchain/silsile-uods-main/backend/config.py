...
class Config:
    ...
    # --- zk-SNARK (Groth16 gizlilik korumalı üyelik ispatı) -----------------
    ZK_ENABLED: bool = _get_bool("ZK_ENABLED", False)
    ZK_DEPTH: int = _get_int("ZK_DEPTH", 10)
    ZK_KEYS_DIR: str = os.getenv("ZK_KEYS_DIR", "instance/zk_keys")
    ZK_VERIFIER_ADDRESS: str = os.getenv("ZK_VERIFIER_ADDRESS", "")
    ...
    @property
    def zk_keys_full_path(self) -> Path:
        p = Path(self.ZK_KEYS_DIR)
        if not p.is_absolute():
            p = self.BASE_DIR / p
        return p
