...
def send_batch_with_zk_to_chain(
    config: Config,
    merkle_root_bytes32: bytes,
    record_count: int,
    zk_root: int,
    ...
    if record_count <= 0:
        raise BlockchainServiceError("record_count pozitif olmalıdır.")
    if zk_root <= 0:
        raise BlockchainServiceError("zk_root pozitif olmalıdır.")
    ...
    try:
        ...
        raise NetworkConnectionError(f"batchExists sorgusu başarısız: {exc}") from exc
    if already_exists:
        raise BatchAlreadyExistsError(f"Merkle kökü zincirde zaten kayıtlı: 0x{root.hex()}")

    nonce = w3.eth.get_transaction_count(account.address, "pending")
    tx = contract.functions.addBatchRootWithZk(
        root, int(record_count), int(zk_root)
    ).build_transaction(
        {
            "from": account.address,
            "nonce": nonce,
...
def set_zk_verifier_on_chain(
    config: Config, verifier_address: str, timeout_seconds: int = 180
) -> str:
    w3 = get_web3(config)
    account = get_institution_account(config)
    contract = get_contract(w3, config)
    ...
    nonce = w3.eth.get_transaction_count(account.address, "pending")
    tx = contract.functions.setZkVerifier(verifier).build_transaction(
        {
            "from": account.address,
            "nonce": nonce,
            "chainId": config.POLYGON_CHAIN_ID,
    ...
    receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=timeout_seconds)
    if receipt.status != 1:
        raise TransactionFailedError(f"setZkVerifier reddedildi, TX: {tx_hash_hex}")
    return tx_hash_hex


def zk_root_exists_on_chain(config: Config, zk_root: int) -> bool:
    w3 = get_web3(config)
    contract = get_contract(w3, config)
    return bool(contract.functions.zkRootExists(int(zk_root)).call())


def verify_zk_membership_on_chain(
    config: Config,
    proof_solidity: dict,
) -> bool:
    w3 = get_web3(config)
    contract = get_contract(w3, config)
    a = [int(x) for x in proof_solidity["a"]]
    b = [[int(x) for x in pair] for pair in proof_solidity["b"]]
    c = [int(x) for x in proof_solidity["c"]]
    public_inputs = [int(x) for x in proof_solidity["publicInputs"]]
    try:
        return bool(
            contract.functions.verifyZkMembership(a, b, c, public_inputs).call()
        )
    except _REVERT_EXCEPTIONS as exc:
        raise BlockchainServiceError(f"verifyZkMembership reddedildi: {exc}") from exc
