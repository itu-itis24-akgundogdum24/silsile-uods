...
class QueueRecord:
    ...
    zk_root: Optional[str] = None
    zk_proof_json: Optional[str] = None
    ...
    @property
    def zk_proof(self) -> Optional[dict]:
        if not self.zk_proof_json:
            return None
        return json.loads(self.zk_proof_json)
...
def init_db(config: Config) -> None:
    ...
    with _connect(config) as conn:
        ...
        if "zk_root" not in existing_cols:
            conn.execute("ALTER TABLE offline_queue ADD COLUMN zk_root TEXT;")
        if "zk_proof_json" not in existing_cols:
            conn.execute("ALTER TABLE offline_queue ADD COLUMN zk_proof_json TEXT;")
...
def _row_to_record(row: sqlite3.Row) -> QueueRecord:
    return QueueRecord(
        ...
        zk_root=row["zk_root"] if "zk_root" in row.keys() else None,
        zk_proof_json=row["zk_proof_json"] if "zk_proof_json" in row.keys() else None,
    )
...
def update_status(
    ...
    if zk_root is not None:
        fields.append("zk_root = ?")
        params.append(zk_root)
    if zk_proof_json is not None:
        fields.append("zk_proof_json = ?")
        params.append(zk_proof_json)
...
class BatchSyncResult:
    ...
    per_record: dict
    zk_root: Optional[str] = None
    # record.id -> zk_proof_json (ZK_ENABLED ise)
    zk_per_record: Optional[dict] = None
    ...
    def run_once(self) -> SyncResult:
        ...
        try:
            ...
            for record in candidates:
                ...
                zk_proof_json = None
                if batch_result.zk_per_record is not None:
                    zk_proof_json = batch_result.zk_per_record.get(record.id)
                update_status(
                    self.config,
                    record.id,
                    STATUS_SYNCED,
