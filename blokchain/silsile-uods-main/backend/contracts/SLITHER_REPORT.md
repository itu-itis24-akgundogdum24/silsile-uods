...
> **v3 notu (zk-SNARK):** Hakem değerlendirmesinin asıl önerisi olan sıfır
> bilgi ispatı, `Groth16Verifier.sol` (BN128 eşleşme precompile'larıyla) ve
> `OpticalFormRegistry.verifyZkMembership(...)` ile FİİLEN UYGULANMIŞTIR.
> Artık bir kaydın bir partiye üyeliği, kaydın kendisi veya ağaçtaki konumu
> İFŞA EDİLMEDEN, zincir üzerinde bir Groth16 ispatıyla doğrulanabilir.
> `verifyZkMembership`, `view` bir fonksiyondur ve yalnızca harici
> doğrulayıcı sözleşmenin saf okuma (`staticcall`) `verifyProof` fonksiyonunu
> çağırır; durum değiştiren hiçbir işlem yapmadığı ve hiçbir değer transferi
> içermediği için yeniden giriş (reentrancy) yüzeyi açmaz.
...
## zk-SNARK doğrulayıcısının (Groth16Verifier.sol) durumu

Hem `OpticalFormRegistry.sol` hem de `Groth16Verifier.sol`, npm üzerindeki
resmî `solc` 0.8.24 paketiyle (optimizasyon açık, 200 tur) **sıfır derleyici
uyarısı ve sıfır hata** ile derlenmektedir. zk doğrulayıcısının matematiksel
doğruluğu, yalnızca statik analizle değil, GERÇEK bir EVM üzerinde uçtan uca
test edilmiştir (bkz. `tests/test_zk_onchain.py`):

- Python tarafında üretilen geçerli bir Groth16 ispatı, zincir üzerindeki
  `verifyZkMembership` tarafından KABUL edilmiştir.
- Bozulmuş (tampered) bir ispat ve kayıtlı olmayan bir zk kökü doğru biçimde
  REDDEDİLMİŞTİR.
- Serileştirilen ispat/VK tamsayılarının BN128 eğrisi üzerinde geçerli
  noktalar oluşturduğu ve Solidity doğrulayıcısının kontrol ettiği tam
  eşleşme denkleminin sağlandığı `py_ecc` ile bağımsız olarak teyit
  edilmiştir (bkz. `tests/test_zk.py`).
