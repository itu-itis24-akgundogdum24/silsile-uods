...
contract OpticalFormRegistry {
    ...
    /// @notice Bir zk Poseidon kökünün bu sözleşmede mühürlenip mühürlenmediğini
    ///         tutar. Zincir üzerinde zk üyelik ispatı doğrulanmadan önce, ilgili
    ///         kökün gerçekten bu kurum tarafından kaydedildiği bu eşlemeyle teyit edilir.
    mapping(uint256 => bool) private zkRoots;

    /// @notice Groth16 zk-SNARK doğrulayıcı sözleşmesinin adresi. Sıfır adres
    ///         ise zk üyelik ispatı özelliği henüz etkin değildir (yalnızca
    ///         keccak Merkle verifyInclusion kullanılır).
    ...
    /// @notice Bir partiye ait zk Poseidon kökü kaydedildiğinde yayınlanır.
    event ZkRootRegistered(bytes32 indexed merkleRoot, uint256 zkRoot);

    /// @notice zk doğrulayıcı sözleşme adresi ayarlandığında yayınlanır.
    ...
    error GecersizAdres();
    error ZkRootBulunamadi(uint256 zkRoot);
    error ZkDogrulayiciTanimsiz();
    ...
    function addBatchRoot(bytes32 merkleRoot, uint256 recordCount) external onlyOwner {
        _addBatch(merkleRoot, recordCount, 0);
    }

    /**
     * @notice keccak Merkle kökünün yanı sıra, aynı partiye ait gizlilik
     *         korumalı zk Poseidon kökünü de aynı işlemde mühürler.
     * @param merkleRoot  keccak Merkle kökü (verifyInclusion için).
     * @param recordCount Partideki kayıt sayısı.
     * @param zkRoot      Poseidon Merkle ağacının kökü (zk üyelik ispatı için
     *                    public girdi). Sıfır olamaz.
     */
    function addBatchRootWithZk(
        bytes32 merkleRoot,
        uint256 recordCount,
        uint256 zkRoot
    ) external onlyOwner {
        if (zkRoot == 0) revert GecersizKayitSayisi();
        _addBatch(merkleRoot, recordCount, zkRoot);
    }

    function _addBatch(bytes32 merkleRoot, uint256 recordCount, uint256 zkRoot) internal {
        ...
        if (zkRoot != 0) {
            zkRoots[zkRoot] = true;
            emit ZkRootRegistered(merkleRoot, zkRoot);
        }
    }

    /**
     * @notice Groth16 zk doğrulayıcı sözleşmesinin adresini ayarlar.
     * @dev    Yalnızca sözleşme sahibi çağırabilir. Sıfır adres reddedilir.
     */
    function setZkVerifier(address verifier) external onlyOwner {
        if (verifier == address(0)) revert GecersizAdres();
        zkVerifier = verifier;
        emit ZkVerifierSet(verifier);
    }

    /// @notice Verilen zk Poseidon kökünün bu sözleşmede kayıtlı olup
    ///         olmadığını döndürür.
    function zkRootExists(uint256 zkRoot) external view returns (bool) {
        return zkRoots[zkRoot];
    }

    /**
     * @notice Bir kaydın, daha önce mühürlenmiş bir zk Poseidon köküne ait
     *         olduğunu, kaydın kendisini veya ağaçtaki konumunu İFŞA ETMEDEN
     *         zincir üzerinde doğrular.
     * @dev    İki koşul birlikte sağlanmalıdır: (1) public girdi olarak verilen
     *         zkRoot bu sözleşmede gerçekten kayıtlı olmalı, (2) Groth16 ispatı
     *         o köke göre geçerli olmalı. publicInputs[0] zkRoot olmalıdır.
     * @return ok  Her iki koşul sağlanıyorsa true.
     */
    function verifyZkMembership(
        ...
        if (zkVerifier == address(0)) revert ZkDogrulayiciTanimsiz();
        if (publicInputs.length == 0) revert GecersizKayitSayisi();
        uint256 zkRoot = publicInputs[0];
        if (!zkRoots[zkRoot]) revert ZkRootBulunamadi(zkRoot);
        return IGroth16Verifier(zkVerifier).verifyProof(a, b, c, publicInputs);
    }
    ...
    /// @notice Verilen merkleRoot'a ait partinin zk Poseidon kökünü döndürür
    ///         (kaydedilmemişse 0). Parti yoksa revert eder.
    function getBatchZkRoot(bytes32 merkleRoot) external view returns (uint256) {
        Batch storage b = batches[merkleRoot];
        if (!b.exists) revert PartiBulunamadi(merkleRoot);
        return b.zkRoot;
    }
