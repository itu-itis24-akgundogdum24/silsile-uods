...
_zk_setup_cache = None


def _get_zk_setup():
    global _zk_setup_cache
    if _zk_setup_cache is None:
        _zk_setup_cache = zk.load_or_generate_setup(
            config.zk_keys_full_path, depth=config.ZK_DEPTH
        )
    return _zk_setup_cache
...
def _sync_batch_records(
    ...
    if config.ZK_ENABLED:
        zk_setup = _get_zk_setup()
        zk_leaves = [zk.sha256_hex_to_fr(r.local_hash) for r in records]
        zk_witnesses = [
            zk.build_membership_witness(zk_leaves, idx, config.ZK_DEPTH)
            for idx in range(len(records))
        ]
        zk_root_int = int(zk_witnesses[0].root)
        receipt = blockchain_service.send_batch_with_zk_to_chain(
            config,
            tree.root,
        ...
        zk_root_hex = str(zk_root_int)
        zk_per_record = {}
        for idx, record in enumerate(records):
            proof = zk.prove_membership(zk_setup, zk_witnesses[idx])
            zk_per_record[record.id] = json.dumps(
                zk.serialize_proof_for_solidity(proof), ensure_ascii=False
            )
    else:
    ...
    # 4) Her kayıt için bireysel Merkle kanıtı üret ve sonucu derle.
    per_record: dict = {}
    for idx, record in enumerate(records):
...
def verify():
    ...
    try:
        ...
        zk_verified = None
        if config.ZK_ENABLED and record.zk_proof_json:
            try:
                zk_verified = blockchain_service.verify_zk_membership_on_chain(
                    config, record.zk_proof
                )
            except blockchain_service.BlockchainServiceError:
                zk_verified = False
